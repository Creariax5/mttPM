...
const PM_API_BASE = process.env.NEXT_PUBLIC_PM_API_URL || 'http://localhost:4058/api/2';

/**
 * Get all prediction market categories
 */
export async function getCategories(): Promise<PMCategory[]> {
  const res = await fetch(`${PM_API_BASE}/pm/categories`);
...
export async function searchMarkets(params: PMSearchParams = {}): Promise<PMSearchResponse> {
  ...
  const res = await fetch(`${PM_API_BASE}/pm/search?${searchParams.toString()}`);
...
export async function getTrendingMarkets(
  ...
  const res = await fetch(`${PM_API_BASE}/pm/trending?${params.toString()}`);
...
export async function getMarketDetails(platform: string, marketId: string): Promise<PMMarket> {
  ...
  const res = await fetch(`${PM_API_BASE}/pm/market/details?${params.toString()}`);
...
export async function getMarketPrices(platform: string, marketId: string) {
  const params = new URLSearchParams({ platform, marketId });
  const res = await fetch(`${PM_API_BASE}/pm/market/price?${params.toString()}`);
...
export async function getMarketOHLCV(
  ...
  const res = await fetch(`${PM_API_BASE}/pm/market/ohlcv?${params.toString()}`);
...
export async function getMarketTrades(
  ...
  const res = await fetch(`${PM_API_BASE}/pm/market/trades?${params.toString()}`);
...
export async function getMarketOrderbook(
  ...
  const res = await fetch(`${PM_API_BASE}/pm/market/orderbook?${params.toString()}`);
